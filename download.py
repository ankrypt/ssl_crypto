...
<Purpose>
  ...
  object, 'ssl_crypto.util.TempFile', is returned by the '_download_file()' function.
"""
...
try:
  ...
  from ssl_crypto._vendor.ssl_match_hostname import match_hostname, CertificateError

# See 'log.py' to learn how logging is handled in TUF.
logger = logging.getLogger('ssl_crypto.download')
...
def unsafe_download(url, required_length):
  ...
  <Purpose>
    ...
    ssl_crypto.download.safe_download() may be called if an exact download limit is
    preferred.
...
def _download_file(url, required_length, STRICT_REQUIRED_LENGTH=True):
  ...
  <Purpose>
    ...
    ssl_crypto.util.TempFile is used instead of regular tempfile object because of 
    additional functionality provided by 'ssl_crypto.util.TempFile'.
  ...
  # the downloaded file.
  temp_file = ssl_crypto.util.TempFile()
...
def _download_fixed_amount_of_data(connection, temp_file, required_length):
  ...
  # 'ssl_crypto.conf.SLOW_START_GRACE_PERIOD' seconds.  Set 'seconds_spent_receiving'
  # to negative SLOW_START_GRACE_PERIOD seconds, and begin checking the average
  # download speed once it is positive.
  grace_period = -ssl_crypto.conf.SLOW_START_GRACE_PERIOD
  ...
  try:
    while True:
      ...
      data = b'' 
      read_amount = min(ssl_crypto.conf.CHUNK_SIZE,
      ...
      if average_download_speed < ssl_crypto.conf.MIN_AVERAGE_DOWNLOAD_SPEED:
        break
...
def _get_opener(scheme=None):
  ...
  if scheme == "https":
    assert os.path.isfile(ssl_crypto.conf.ssl_certificates)
...
def _open_connection(url):
  ...
  return opener.open(request, timeout = ssl_crypto.conf.SOCKET_TIMEOUT)
...
def _check_downloaded_length(total_downloaded, required_length,
  ...
  <Exceptions>
    ssl_crypto.DownloadLengthMismatchError, if STRICT_REQUIRED_LENGTH is True and
  ...
  if total_downloaded == required_length:
    ...
    if STRICT_REQUIRED_LENGTH:
      ...
      raise ssl_crypto.DownloadLengthMismatchError(required_length, total_downloaded)
...
class VerifiedHTTPSConnection(six.moves.http_client.HTTPSConnection):
  ...
  def connect(self):
    ...
    assert os.path.isfile(ssl_crypto.conf.ssl_certificates)
    cert_path = ssl_crypto.conf.ssl_certificates
