...
def get_list_of_mirrors(file_type, file_path, mirrors_dict):
  ...
  <Exceptions>
    ssl_crypto.Error, on unsupported 'file_type'.
    
    ssl_crypto.FormatError, on bad argument.
  ...
  ssl_crypto.formats.RELPATH_SCHEMA.check_match(file_path)
  ssl_crypto.formats.MIRRORDICT_SCHEMA.check_match(mirrors_dict)
  ssl_crypto.formats.NAME_SCHEMA.check_match(file_type)
  ...
  # Reference to 'ssl_crypto.util.file_in_confined_directories()' (improve readability).
  ...
  # the repository mirror is added to the 'ssl_crypto.client.updater.Updater' object.
  in_confined_directory = ssl_crypto.util.file_in_confined_directories
