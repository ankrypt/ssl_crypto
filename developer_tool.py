...
<Purpose>
  See 'ssl_crypto/README-developer-tools.md' for a complete guide on using
  'developer_tool.py'.
"""
...
import ssl_crypto.repository_tool
import six

# These imports provide the interface for 'developer_tool.py', since the imports
# are made there. 
from ssl_crypto.keys import format_keyval_to_metadata
from ssl_crypto.keys import format_metadata_to_key

from ssl_crypto.repository_tool import Targets
from ssl_crypto.repository_lib import get_metadata_fileinfo
from ssl_crypto.repository_lib import get_metadata_filenames
from ssl_crypto.repository_tool import generate_and_write_rsa_keypair
from ssl_crypto.repository_tool import import_rsa_publickey_from_file
from ssl_crypto.repository_tool import import_rsa_privatekey_from_file
from ssl_crypto.repository_tool import generate_and_write_ed25519_keypair
from ssl_crypto.repository_tool import import_ed25519_publickey_from_file
from ssl_crypto.repository_tool import import_ed25519_privatekey_from_file
from ssl_crypto.repository_lib import _remove_invalid_and_duplicate_signatures
from ssl_crypto.repository_lib import disable_console_log_messages 
from ssl_crypto.repository_lib import _check_role_keys
from ssl_crypto.repository_lib import _delete_obsolete_metadata
from ssl_crypto.repository_lib import generate_targets_metadata
from ssl_crypto.repository_lib import sign_metadata
from ssl_crypto.repository_lib import write_metadata_file
from ssl_crypto.repository_lib import _metadata_is_partially_loaded

# See 'log.py' to learn how logging is handled in TUF.
logger = logging.getLogger('ssl_crypto.developer_tool')
...
from ssl_crypto.repository_lib import DEFAULT_RSA_KEY_BITS as DEFAULT_RSA_KEY_BITS

# The algorithm used by the developer tools to generate the hashes of the
# target filepaths. 
from ssl_crypto.repository_tool import HASH_FUNCTION as HASH_FUNCTION

# The extension of TUF metadata.
from ssl_crypto.repository_lib  import METADATA_EXTENSION as METADATA_EXTENSION

# The metadata filename for the targets metadata information.
from ssl_crypto.repository_lib import TARGETS_FILENAME as TARGETS_FILENAME
...
from ssl_crypto.repository_tool import METADATA_STAGED_DIRECTORY_NAME
from ssl_crypto.repository_tool import METADATA_DIRECTORY_NAME
from ssl_crypto.repository_tool import TARGETS_DIRECTORY_NAME

# The full list of supported TUF metadata extensions.
from ssl_crypto.repository_lib import METADATA_EXTENSIONS

# The recognized compression extensions. 
from ssl_crypto.repository_lib import SUPPORTED_COMPRESSION_EXTENSIONS

# Supported key types.
from ssl_crypto.repository_lib import SUPPORTED_KEY_TYPES
...
class Project(Targets):
  ...
  def __init__(self, project_name, metadata_directory, targets_directory,
    ...
    ssl_crypto.formats.NAME_SCHEMA.check_match(project_name)
    ...
    ssl_crypto.formats.PATH_SCHEMA.check_match(file_prefix)
  ...
  def write(self, write_partial=False):
    ...
    <Exceptions>
      ssl_crypto.Error, if any of the project roles do not have a minimum threshold of
      signatures.
    ...
    # populated, otherwise write() throwns a 'ssl_crypto.Repository' exception if 
    ...
    delegated_rolenames = \
      ssl_crypto.roledb.get_delegated_rolenames(self._project_name)
  ...
  def add_verification_key(self, key):
    """
      ...
      <Exceptions>
        ...
        ssl_crypto.Error, if the project already contains a key.
    ...
    if len(self.keys) > 0:
      raise ssl_crypto.Error("This project already contains a key.")

    try:
      super(Project, self).add_verification_key(key)
    
    except ssl_crypto.FormatError:
      raise
  ...
  def status(self):
    ...
    <Exceptions>
      ssl_crypto.Error, if the project, or any of its delegated roles, do not have a
    ...
    try:
      ...
      # Delegated roles.
      delegated_roles = ssl_crypto.roledb.get_delegated_rolenames(self._project_name)
      insufficient_keys = []
      ...
      for delegated_role in delegated_roles:
        ...
        roleinfo = ssl_crypto.roledb.get_roleinfo(delegated_role)
        try: 
...
def _generate_and_write_metadata(rolename, metadata_filename, write_partial,
  ...
  # 'signable' contains an invalid threshold of signatures. 
    ...
    raise ssl_crypto.Error(message, signable)
...
def create_new_project(project_name, metadata_directory,
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the arguments are improperly formatted or if the public
  ...
  <Returns>
    A 'ssl_crypto.developer_tool.Project' object.
  """
  ...
  ssl_crypto.formats.NAME_SCHEMA.check_match(project_name)
  ssl_crypto.formats.PATH_SCHEMA.check_match(location_in_repository)
  ...
  if key is not None:
    ssl_crypto.formats.KEY_SCHEMA.check_match(key)
...
def _save_project_configuration(metadata_directory, targets_directory,
  ...
  <Exceptions>
    ssl_crypto.FormatError are also expected if any of the arguments are malformed.
  ...
  ssl_crypto.formats.PATH_SCHEMA.check_match(metadata_directory)
  ssl_crypto.formats.PATH_SCHEMA.check_match(prefix)
  ssl_crypto.formats.PATH_SCHEMA.check_match(targets_directory)
  ssl_crypto.formats.RELPATH_SCHEMA.check_match(project_name)
  ...
  for key in public_keys:
    key_info = ssl_crypto.keydb.get_key(key)
...
def load_project(project_directory, prefix='', new_targets_location=None):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'project_directory' or any of the metadata files
    are improperly formatted. 

  <Side Effects>
    All the metadata files found in the project are loaded and their contents
    stored in a libssl_crypto.Repository object.
  ...
  try:
    project_configuration = ssl_crypto.util.load_json_file(config_filename)
    ssl_crypto.formats.PROJECT_CFG_SCHEMA.check_match(project_configuration) 
  ...
  targets_metadata_path = os.path.join(project_directory, metadata_directory,
      project_filename)
  signable = ssl_crypto.util.load_json_file(targets_metadata_path)
  ...
  roleinfo = ssl_crypto.roledb.get_roleinfo(project_name)
  ...
  ssl_crypto.roledb.update_roleinfo(project_name, roleinfo)
  ...
  if os.path.exists(targets_metadata_directory) and \
    ...
    for root, directories, files in os.walk(targets_metadata_directory):
      ...
      for basename in files:
        ...
        except (ValueError, IOError, ssl_crypto.Error):
          raise
