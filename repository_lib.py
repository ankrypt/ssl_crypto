...
<Purpose>
  ...
  mode, or imported directly into a Python module.  See 'ssl_crypto/README' for the
  complete guide to using 'ssl_crypto.repository_tool.py'.
"""
...
import ssl_crypto.roledb
import ssl_crypto.keys
import ssl_crypto.sig
...
logger = logging.getLogger('ssl_crypto.repository_lib')
...
def _generate_and_write_metadata(rolename, metadata_filename, write_partial,
  ...
  roleinfo = ssl_crypto.roledb.get_roleinfo(rolename) 
  ...
  # Check for the Targets role, including delegated roles.
    ...
    raise ssl_crypto.Error('Invalid rolename') 
  ...
  if write_partial:
    ...
    status = ssl_crypto.sig.get_signature_status(temp_signable, rolename)
    if len(status['good_sigs']) == 0:
      metadata['version'] = metadata['version'] + 1
      roleinfo = ssl_crypto.roledb.get_roleinfo(rolename)
      roleinfo['version'] = roleinfo['version'] + 1
      ssl_crypto.roledb.update_roleinfo(rolename, roleinfo)
      signable = sign_metadata(metadata, roleinfo['signing_keyids'],
                               metadata_filename)
  # non-partial write()
  else:
    ...
    # the latest version).
    if ssl_crypto.sig.verify(signable, rolename) and not roleinfo['partial_loaded']:
      metadata['version'] = metadata['version'] + 1
      roleinfo = ssl_crypto.roledb.get_roleinfo(rolename)
      roleinfo['version'] = roleinfo['version'] + 1
      ssl_crypto.roledb.update_roleinfo(rolename, roleinfo) 
  ...
  if ssl_crypto.sig.verify(signable, rolename) or write_partial:
  ...
  # 'signable' contains an invalid threshold of signatures. 
    ...
    raise ssl_crypto.UnsignedMetadataError(message, signable)
...
def _metadata_is_partially_loaded(rolename, signable, roleinfo):
  ...
  its 'ssl_crypto.roledb' roleinfo.  This function exists to assist in deciding whether
  ...
  status = ssl_crypto.sig.get_signature_status(signable, rolename)
...
def _check_directory(directory):
  ...
  <Exceptions>
    ssl_crypto.Error, if 'directory' could not be validated.

    ssl_crypto.FormatError, if 'directory' is not properly formatted.
  ...
  ssl_crypto.formats.PATH_SCHEMA.check_match(directory)

  # Check if the directory exists.
  if not os.path.isdir(directory):
    raise ssl_crypto.Error(repr(directory) + ' directory does not exist.')
...
def _check_role_keys(rolename):
  ...
  if total_keyids < threshold: 
    ...
    raise ssl_crypto.InsufficientKeysError(message)
...
def _remove_invalid_and_duplicate_signatures(signable):
  ...
  for signature in signable['signatures']:
    ...
    # in 'ssl_crypto.keydb'.
    try:
      key = ssl_crypto.keydb.get_key(keyid)
    
    except ssl_crypto.UnknownKeyError as e:
      signable['signatures'].remove(signature)
      continue
    
    # Remove 'signature' from 'signable' if it is an invalid signature.
    if not ssl_crypto.keys.verify_signature(key, signature, signed):
      signable['signatures'].remove(signature)
  ...
  if os.path.exists(targets_metadata) and os.path.isdir(targets_metadata):
    for directory_path, junk_directories, files in os.walk(targets_metadata):
      ...
      for basename in files:
        ...
        # Delete the metadata file if it does not exist in 'ssl_crypto.roledb'.
        # 'repository_tool.py' might have removed 'metadata_name,'
        # but its metadata file is not actually deleted yet.  Do it now.
        if not ssl_crypto.roledb.role_exists(metadata_name):
...
def _load_top_level_metadata(repository, top_level_filenames):
  ...
  if os.path.exists(root_filename):
    ...
    signable = ssl_crypto.util.load_json_file(root_filename)
    ssl_crypto.formats.check_signable_object_format(signable)
    root_metadata = signable['signed']  
    ssl_crypto.keydb.create_keydb_from_root_metadata(root_metadata)
    ssl_crypto.roledb.create_roledb_from_root_metadata(root_metadata)

    # Load Root's roleinfo and update 'ssl_crypto.roledb'.
    roleinfo = ssl_crypto.roledb.get_roleinfo('root')
    roleinfo['signatures'] = []
    for signature in signable['signatures']:
    ...
    ssl_crypto.roledb.update_roleinfo('root', roleinfo)
    ...
    raise ssl_crypto.RepositoryError(message)
  
  # Load 'timestamp.json'.  A Timestamp role file without a version number is
  # always written. 
  if os.path.exists(timestamp_filename):
    signable = ssl_crypto.util.load_json_file(timestamp_filename)
    timestamp_metadata = signable['signed']  
    for signature in signable['signatures']:
      repository.timestamp.add_signature(signature)

    # Load Timestamp's roleinfo and update 'ssl_crypto.roledb'.
    roleinfo = ssl_crypto.roledb.get_roleinfo('timestamp')
    ...
    ssl_crypto.roledb.update_roleinfo('timestamp', roleinfo)
  ...
  if os.path.exists(snapshot_filename):
    signable = ssl_crypto.util.load_json_file(snapshot_filename)
    ssl_crypto.formats.check_signable_object_format(signable)
    snapshot_metadata = signable['signed']  
    for signature in signable['signatures']:
      repository.snapshot.add_signature(signature)

    # Load Snapshot's roleinfo and update 'ssl_crypto.roledb'.
    roleinfo = ssl_crypto.roledb.get_roleinfo('snapshot')
    ...
    ssl_crypto.roledb.update_roleinfo('snapshot', roleinfo)
  ...
  if os.path.exists(targets_filename):
    signable = ssl_crypto.util.load_json_file(targets_filename)
    ssl_crypto.formats.check_signable_object_format(signable)
    ...
    # Update 'targets.json' in 'ssl_crypto.roledb.py' 
    roleinfo = ssl_crypto.roledb.get_roleinfo('targets')
    ...
    ssl_crypto.roledb.update_roleinfo('targets', roleinfo)

    # Add the keys specified in the delegations field of the Targets role.
    for key_metadata in six.itervalues(targets_metadata['delegations']['keys']):
      key_object = ssl_crypto.keys.format_metadata_to_key(key_metadata)
      ...
      try: 
        ssl_crypto.keydb.add_key(key_object)
      
      except ssl_crypto.KeyAlreadyExistsError as e:
        pass
    ...
    for role in targets_metadata['delegations']['roles']:
      ...
      roleinfo = {'name': role['name'], 'keyids': role['keyids'],
                  ...
                  'signatures': [], 'delegations': {'keys': {},
                                                    'roles': []}}
      ssl_crypto.roledb.add_role(rolename, roleinfo)
...
def _log_warning_if_expires_soon(rolename, expires_iso8601_timestamp,
  ...
  expires_unix_timestamp = \
    ssl_crypto.formats.datetime_to_unix_timestamp(datetime_object) 
...
def generate_and_write_rsa_keypair(filepath, bits=DEFAULT_RSA_KEY_BITS,
  ...
  <Purpose>
    ...
    set in 'ssl_crypto.conf.RSA_CRYPTO_LIBRARY'.  PyCrypto currently supported.  The
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the arguments are improperly formatted.
  ...
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.PATH_SCHEMA.check_match(filepath)

  # Does 'bits' have the correct format?
  ssl_crypto.formats.RSAKEYBITS_SCHEMA.check_match(bits)
  ...
  rsa_key = ssl_crypto.keys.generate_rsa_key(bits)
  public = rsa_key['keyval']['public']
  private = rsa_key['keyval']['private']
  encrypted_pem = ssl_crypto.keys.create_rsa_encrypted_pem(private, password) 
  ...
  ssl_crypto.util.ensure_parent_dir(filepath)

  # Create a tempororary file, write the contents of the public key, and move
  # to final destination.
  file_object = ssl_crypto.util.TempFile()
...
def import_rsa_privatekey_from_file(filepath, password=None):
  ...
  <Purpose>
    ...
    object in 'ssl_crypto.formats.RSAKEY_SCHEMA' format.

    Which cryptography library performs the cryptographic decryption is
    determined by the string set in 'ssl_crypto.conf.RSA_CRYPTO_LIBRARY'.  PyCrypto
    currently supported.
  ...
  <Exceptions>
    ...
    ssl_crypto.CryptoError, if 'filepath' is not a valid encrypted key file.

  <Side Effects>
    The contents of 'filepath' is read, decrypted, and the key stored.

  <Returns>
    An RSA key object, conformant to 'ssl_crypto.formats.RSAKEY_SCHEMA'.
  """
  ...
  # Convert 'encrypted_pem' to 'ssl_crypto.formats.RSAKEY_SCHEMA' format.  Raise
  # 'ssl_crypto.CryptoError' if 'encrypted_pem' is invalid.
  rsa_key = ssl_crypto.keys.import_rsakey_from_encrypted_pem(encrypted_pem, password)
...
def import_rsa_publickey_from_file(filepath):
  ...
  <Purpose>
    ...
    key, specifically 'ssl_crypto.formats.RSAKEY_SCHEMA'.  If the RSA PEM in 'filepath'
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'filepath' is improperly formatted.

    ssl_crypto.Error, if a valid RSA key object cannot be generated.  This may be
  ...
  # Convert 'rsa_pubkey_pem' to 'ssl_crypto.formats.RSAKEY_SCHEMA' format.
  try:
    rsakey_dict = ssl_crypto.keys.format_rsakey_from_pem(rsa_pubkey_pem)
  
  except ssl_crypto.FormatError as e:
    raise ssl_crypto.Error('Cannot import improperly formatted PEM file.')
...
def generate_and_write_ed25519_keypair(filepath, password=None):
  ...
  <Purpose>
    ...
    library performs the cryptographic decryption is determined by the string
    set in 'ssl_crypto.conf.ED25519_CRYPTO_LIBRARY'.
  ...
  <Exceptions>
    ...
    ssl_crypto.CryptoError, if 'filepath' cannot be encrypted.

    ssl_crypto.UnsupportedLibraryError, if 'filepath' cannot be encrypted due to an
    invalid configuration setting (i.e., invalid 'ssl_crypto.conf.py' setting).
  ...
  # 'ssl_crypto.conf.ED25519_CRYPTO_LIBRARY').  Raise 'ssl_crypto.CryptoError' or
  # 'ssl_crypto.UnsupportedLibraryError', if 'ed25519_key' cannot be encrypted.
  ed25519_key = ssl_crypto.keys.generate_ed25519_key()
  encrypted_key = ssl_crypto.keys.encrypt_key(ed25519_key, password) 
  ...
  ed25519key_metadata_format = \
    ssl_crypto.keys.format_keyval_to_metadata(keytype, keyval, private=False)
  
  # Write the public key, conformant to 'ssl_crypto.formats.KEY_SCHEMA', to
  # '<filepath>.pub'.
  ssl_crypto.util.ensure_parent_dir(filepath)
  ...
  # 'ssl_crypto.formats.ENCRYPTEDKEY_SCHEMA', to '<filepath>'.
  file_object = ssl_crypto.util.TempFile()
...
def import_ed25519_publickey_from_file(filepath):
  """
  <Purpose>
    Load the ED25519 public key object (conformant to 'ssl_crypto.formats.KEY_SCHEMA')
    stored in 'filepath'.  Return 'filepath' in ssl_crypto.formats.ED25519KEY_SCHEMA
    format.
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'filepath' is improperly formatted or is an unexpected
    key type.
  ...
  <Returns>
    An ED25519 key object conformant to 'ssl_crypto.formats.ED25519KEY_SCHEMA'.
  """
  ...
  # loaded key object in ssl_crypto.formats.ED25519KEY_SCHEMA' format that also
  # includes the keyid.
  ed25519_key_metadata = ssl_crypto.util.load_json_file(filepath)
  ed25519_key = ssl_crypto.keys.format_metadata_to_key(ed25519_key_metadata)
  
  # Raise an exception if an unexpected key type is imported.
  # Redundant validation of 'keytype'.  'ssl_crypto.keys.format_metadata_to_key()'
...
def import_ed25519_privatekey_from_file(filepath, password=None):
  ...
  <Purpose>
    ...
    return the key object in 'ssl_crypto.formats.ED25519KEY_SCHEMA' format.

    Which cryptography library performs the cryptographic decryption is
    determined by the string set in 'ssl_crypto.conf.ED25519_CRYPTO_LIBRARY'.  PyCrypto
    currently supported.
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the arguments are improperly formatted or the imported
    key object contains an invalid key type (i.e., not 'ed25519').

    ssl_crypto.CryptoError, if 'filepath' cannot be decrypted.

    ssl_crypto.UnsupportedLibraryError, if 'filepath' cannot be decrypted due to an
    invalid configuration setting (i.e., invalid 'ssl_crypto.conf.py' setting).
  ...
  <Returns>
    An ed25519 key object of the form: 'ssl_crypto.formats.ED25519KEY_SCHEMA'.
  """
  ...
  # 'password'.  Raise 'ssl_crypto.CryptoError' or 'ssl_crypto.UnsupportedLibraryError' if the
  # decryption fails.
  key_object = ssl_crypto.keys.decrypt_key(encrypted_key, password)
...
def get_metadata_filenames(metadata_directory=None):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'metadata_directory' is improperly formatted.
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.PATH_SCHEMA.check_match(metadata_directory)
...
def get_metadata_fileinfo(filename, custom=None):
  ...
  <Purpose>
    ...
    conforms to 'ssl_crypto.formats.FILEINFO_SCHEMA'.  The information
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'filename' is improperly formatted.

    ssl_crypto.Error, if 'filename' doesn't exist.
  ...
  <Returns>
    A dictionary conformant to 'ssl_crypto.formats.FILEINFO_SCHEMA'.  This
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.PATH_SCHEMA.check_match(filename)
  if custom is not None:
    ssl_crypto.formats.CUSTOM_SCHEMA.check_match(custom)

  if not os.path.isfile(filename):
    message = repr(filename) + ' is not a file.'
    raise ssl_crypto.Error(message)
  ...
  # numbers, etc.
  filesize, filehashes = \
    ssl_crypto.util.get_file_details(filename, ssl_crypto.conf.REPOSITORY_HASH_ALGORITHMS)

  return ssl_crypto.formats.make_fileinfo(filesize, filehashes, custom)
...
def get_metadata_versioninfo(rolename):
  ...
  <Purpose>
    ...
    conforms to 'ssl_crypto.formats.VERSIONINFO_SCHEMA'.  The information
  ...
  <Arguments>
    rolename:
      ...
      a 'ssl_crypto.UnknownRoleError' exception is raised.

  <Exceptions>
    ssl_crypto.FormatError, if 'rolename' is improperly formatted.

    ssl_crypto.UnknownRoleError, if 'rolename' does not exist.

  <Side Effects>
    None.
  
  <Returns>
    A dictionary conformant to 'ssl_crypto.formats.VERSIONINFO_SCHEMA'.  This
...
def get_target_hash(target_filepath):
  ...
  return ssl_crypto.util.get_target_hash(target_filepath)
...
def generate_root_metadata(version, expiration_date, consistent_snapshot,
  ...
  """
  <Purpose>
    Create the root metadata.  'ssl_crypto.roledb.py' and 'ssl_crypto.keydb.py' are read and
  ...
  <Arguments>
    ...
    expiration_date:
      ...
      'ssl_crypto.formats.ISO8601_DATETIME_SCHEMA'.
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the generated root metadata object could not
    be generated with the correct format.

    ssl_crypto.Error, if an error is encountered while generating the root
    metadata object (e.g., a required top-level role not found in 'ssl_crypto.roledb'.)
  
  <Side Effects>
    The contents of 'ssl_crypto.keydb.py' and 'ssl_crypto.roledb.py' are read.

  <Returns>
    A root metadata object, conformant to 'ssl_crypto.formats.ROOT_SCHEMA'.
  """

  # Do the arguments have the correct format?
  # Ensure the arguments have the appropriate number of objects and object
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if any of the arguments are improperly formatted.
  ssl_crypto.formats.METADATAVERSION_SCHEMA.check_match(version)
  ssl_crypto.formats.ISO8601_DATETIME_SCHEMA.check_match(expiration_date)
  ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(consistent_snapshot)
  ssl_crypto.formats.COMPRESSIONS_SCHEMA.check_match(compression_algorithms)
  ...
  for rolename in ['root', 'targets', 'snapshot', 'timestamp']:
    ...
    # If a top-level role is missing from 'ssl_crypto.roledb.py', raise an exception.
    if not ssl_crypto.roledb.role_exists(rolename):
      raise ssl_crypto.Error(repr(rolename) + ' not in "ssl_crypto.roledb".')
   
    # Keep track of the keys loaded to avoid duplicates.
    keyids = []

    # Generate keys for the keyids listed by the role being processed.
    for keyid in ssl_crypto.roledb.get_role_keyids(rolename):
      key = ssl_crypto.keydb.get_key(keyid)

      # If 'key' is an RSA key, it would conform to 'ssl_crypto.formats.RSAKEY_SCHEMA',
      # and have the form:
      # {'keytype': 'rsa',
      #  'keyid': keyid,
      ...
      if keyid not in keydict:
        ...
        # This is not a recognized key.  Raise an exception.
          ...
          raise ssl_crypto.Error('Unsupported keytype: '+keyid)
      
      # Do we have a duplicate?
      if keyid in keyids:
        raise ssl_crypto.Error('Same keyid listed twice: '+keyid)
      
      # Add the loaded keyid for the role being processed.
      keyids.append(keyid)
    
    # Generate and store the role data belonging to the processed role.
    role_threshold = ssl_crypto.roledb.get_role_threshold(rolename)
    role_metadata = ssl_crypto.formats.make_role_metadata(keyids, role_threshold)
    roledict[rolename] = role_metadata

  # Generate the root metadata object.
  root_metadata = ssl_crypto.formats.RootFile.make_metadata(version, expiration_date,
                                                     keydict, roledict,
                                                     consistent_snapshot,
                                                     compression_algorithms)
...
def generate_targets_metadata(targets_directory, target_files, version,
  ...
  <Arguments>
    ...
    delegations:
      ...
      must match 'ssl_crypto.formats.DELEGATIONS_SCHEMA'.
  ...
  <Exceptions>
    ssl_crypto.FormatError, if an error occurred trying to generate the targets
    metadata object.

    ssl_crypto.Error, if any of the target files cannot be read. 

  <Side Effects>
    The target files are read and file information generated about them.

  <Returns>
    A targets metadata object, conformant to 'ssl_crypto.formats.TARGETS_SCHEMA'.
  """
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.PATH_SCHEMA.check_match(targets_directory)
  ssl_crypto.formats.PATH_FILEINFO_SCHEMA.check_match(target_files)
  ssl_crypto.formats.METADATAVERSION_SCHEMA.check_match(version)
  ssl_crypto.formats.ISO8601_DATETIME_SCHEMA.check_match(expiration_date)
  ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(write_consistent_targets)

  if delegations is not None:
    ssl_crypto.formats.DELEGATIONS_SCHEMA.check_match(delegations)
  
  # Store the file attributes of targets in 'target_files'.  'filedict',
  # conformant to 'ssl_crypto.formats.FILEDICT_SCHEMA', is added to the targets
  ...
  targets_metadata = ssl_crypto.formats.TargetsFile.make_metadata(version,
                                                           expiration_date,
                                                           filedict,
                                                           delegations)
...
def generate_snapshot_metadata(metadata_directory, version, expiration_date,
  ...
  <Arguments>
    ...
    expiration_date:
      ...
      Conformant to 'ssl_crypto.formats.ISO8601_DATETIME_SCHEMA'.
  ...
  <Exceptions>
    ...
    ssl_crypto.Error, if an error occurred trying to generate the snapshot metadata
    object.
  ...
  <Returns>
    The snapshot metadata object, conformant to 'ssl_crypto.formats.SNAPSHOT_SCHEMA'.
  """
  ...
  ssl_crypto.formats.METADATAVERSION_SCHEMA.check_match(version)
  ssl_crypto.formats.ISO8601_DATETIME_SCHEMA.check_match(expiration_date)
  ssl_crypto.formats.PATH_SCHEMA.check_match(root_filename)
  ssl_crypto.formats.PATH_SCHEMA.check_match(targets_filename)
  ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(consistent_snapshot)
      ...
      for basename in files:
        ...
        for metadata_extension in METADATA_EXTENSIONS: 
          if metadata_name.endswith(metadata_extension):
            ...
            if ssl_crypto.roledb.role_exists(rolename):
              versiondict[metadata_name] = get_metadata_versioninfo(rolename)

  # Generate the Snapshot metadata object.
  snapshot_metadata = ssl_crypto.formats.SnapshotFile.make_metadata(version,
                                                             expiration_date,
                                                             versiondict)
...
def generate_timestamp_metadata(snapshot_filename, version, expiration_date):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the generated timestamp metadata object cannot be
  ...
  <Returns>
    A timestamp metadata object, conformant to 'ssl_crypto.formats.TIMESTAMP_SCHEMA'.
  """
  ...
  # object types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if the check fails.
  ssl_crypto.formats.PATH_SCHEMA.check_match(snapshot_filename)
  ssl_crypto.formats.METADATAVERSION_SCHEMA.check_match(version)
  ssl_crypto.formats.ISO8601_DATETIME_SCHEMA.check_match(expiration_date)
  ...
  timestamp_metadata = ssl_crypto.formats.TimestampFile.make_metadata(version,
                                                               expiration_date,
                                                               versioninfo)
...
def sign_metadata(metadata_object, keyids, filename):
  ...
  <Purpose>
    ...
    loaded in 'ssl_crypto.keydb'.

  <Arguments>
    metadata_object:
      The metadata object to sign.  For example, 'metadata' might correspond to
      'ssl_crypto.formats.ROOT_SCHEMA' or 'ssl_crypto.formats.TARGETS_SCHEMA'.
  ...
  <Exceptions>
    ssl_crypto.FormatError, if a valid 'signable' object could not be generated or
    the arguments are improperly formatted.

    ssl_crypto.Error, if an invalid keytype was found in the keystore. 
  
  <Side Effects>
    None.

  <Returns>
    A signable object conformant to 'ssl_crypto.formats.SIGNABLE_SCHEMA'.
  """
  ...
  # object types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if the check fails.
  ssl_crypto.formats.ANYROLE_SCHEMA.check_match(metadata_object)  
  ssl_crypto.formats.KEYIDS_SCHEMA.check_match(keyids)
  ssl_crypto.formats.PATH_SCHEMA.check_match(filename)
  ...
  # keyid of 'keyids'.
  signable = ssl_crypto.formats.make_signable(metadata_object)
  ...
  for keyid in keyids:
    ...
    if key['keytype'] in SUPPORTED_KEY_TYPES:
      if 'private' in key['keyval']:
        signed = signable['signed']
        signature = ssl_crypto.keys.create_signature(key, signed)
      ...
      raise ssl_crypto.Error('The keydb contains a key with an invalid key type.')

  # Raise 'ssl_crypto.FormatError' if the resulting 'signable' is not formatted
  # correctly.
  ssl_crypto.formats.check_signable_object_format(signable)
...
def write_metadata_file(metadata, filename, version_number,
  ...
  <Exceptions>
    ...
    ssl_crypto.Error, if the directory of 'filename' does not exist.
  ...
  # object types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if the check fails.
  ssl_crypto.formats.SIGNABLE_SCHEMA.check_match(metadata)
  ssl_crypto.formats.PATH_SCHEMA.check_match(filename)
  ssl_crypto.formats.METADATAVERSION_SCHEMA.check_match(version_number)
  ssl_crypto.formats.COMPRESSIONS_SCHEMA.check_match(compression_algorithms)
  ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(consistent_snapshot)
  ...
  new_digests = {}
  hash_algorithms = ssl_crypto.conf.REPOSITORY_HASH_ALGORITHMS
  for hash_algorithm in hash_algorithms: 
    digest_object = ssl_crypto.hash.digest(hash_algorithm)
    digest_object.update(file_content)
    new_digests.update({hash_algorithm: digest_object.hexdigest()})

  try:
    file_length_junk, old_digests = ssl_crypto.util.get_file_details(written_filename)
    if old_digests != new_digests:
      write_new_metadata = True
  
  # 'ssl_crypto.Error' raised if 'filename' does not exist.
  ...
  if write_new_metadata:
    ...
    # and indentation is used.  The 'ssl_crypto.util.TempFile' file-like object is
  ...
  for compression_algorithm in compression_algorithms:
    ...
    if not len(compression_algorithm):
      ...
      raise ssl_crypto.FormatError('Unknown compression algorithm: ' + repr(compressio_algorithm))
...
def _write_compressed_metadata(file_object, compressed_filename,
  ...
  if not consistent_snapshot:
    ...
    # ssl_crypto.util.TempFile() automatically closes the temp file when move() is
    # called
    else:
  ...
  # compressed filename written, provided it does not already exist.
    ...
    for hash_algorithm in ssl_crypto.conf.REPOSITORY_HASH_ALGORITHMS:
      digest_object = ssl_crypto.hash.digest(hash_algorithm)
    ...
    # Move the 'ssl_crypto.util.TempFile' object to one of the filenames so that it is
...
def _log_status_of_top_level_roles(targets_directory, metadata_directory):
  ...
  for rolename in ['root', 'targets', 'snapshot', 'timestamp']:
    ...
    except ssl_crypto.InsufficientKeysError as e:
      logger.info(str(e))
      return
  ...
  # 'ssl_crypto.UnsignedMetadataError' raised if metadata contains an invalid threshold
  # of signatures.  log the valid/threshold message, where valid < threshold.
  except ssl_crypto.UnsignedMetadataError as e:
...
def create_ssl_crypto_client_directory(repository_directory, client_directory):
  """
  <Purpose>
    Create a client directory structure that the 'ssl_crypto.interposition' package
    and 'ssl_crypto.client.updater' module expect of clients.  Metadata files
    ...
    satisfied.  create_ssl_crypto_client_directory() ensures the minimum metadata
  ...
  <Exceptions>
    ...
    ssl_crypto.RepositoryError, if the metadata directory in 'client_directory'
    already exists.
  ...
  # object types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if the check fails.
  ssl_crypto.formats.PATH_SCHEMA.check_match(repository_directory)
  ssl_crypto.formats.PATH_SCHEMA.check_match(client_directory)
  ...
  # 'ssl_crypto.interposition' and 'ssl_crypto.client.updater.py' expect the 'current' and
...
def disable_console_log_messages():
  ...
  <Side Effects>
    Removes the 'ssl_crypto.log' console handler, added by default when
    'ssl_crypto.repository_tool.py' is imported.
  
  <Returns>
    None.
  """
  
  ssl_crypto.log.remove_console_handler()
