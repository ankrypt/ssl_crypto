...
def get_signature_status(signable, role=None):
  ...
  <Purpose>
    ...
    of public keys in 'ssl_crypto.keydb', a set of roles in 'ssl_crypto.roledb',
    ...
    signable:
      ...
      signable = {'signed': 'signer',
                  'signatures': [{'keyid': keyid,
                                  'method': 'evp',
                                  'sig': sig}]}
      Conformant to ssl_crypto.formats.SIGNABLE_SCHEMA.

    role:
      TUF role (e.g., 'root', 'targets', 'snapshot').

  <Exceptions>
    ssl_crypto.FormatError, if 'signable' does not have the correct format.

    ssl_crypto.UnknownRoleError, if 'role' is not recognized.
  ...
  <Returns>
    ...
    Conformant to ssl_crypto.formats.SIGNATURESTATUS_SCHEMA.
  """
  ...
  ssl_crypto.formats.SIGNABLE_SCHEMA.check_match(signable)
  ...
  for signature in signatures:
    ...
    except ssl_crypto.UnknownKeyError:
      unknown_sigs.append(keyid)
      continue

    # Identify key using an unknown key signing method.
    try:
      valid_sig = ssl_crypto.keys.verify_signature(key, signature, signed)
    
    except ssl_crypto.UnknownMethodError:
    ...
    if valid_sig:
      if role is not None:
        try:
          ...
          if keyid not in ssl_crypto.roledb.get_role_keyids(role):
  ...
  # Retrieve the threshold value for 'role'.  Raise ssl_crypto.UnknownRoleError
  # if we were given an invalid role.
  if role is not None:
    try:
      threshold = ssl_crypto.roledb.get_role_threshold(role)
...
def verify(signable, role):
  ...
  <Exceptions>
    ...
    ssl_crypto.FormatError, if 'signable' is not formatted correctly.

    ssl_crypto.Error, if an invalid threshold is encountered.

  <Side Effects>
    ssl_crypto.sig.get_signature_status() called.  Any exceptions thrown by
  ...
  # Retrieve the signature status.  ssl_crypto.sig.get_signature_status() raises
  # ssl_crypto.UnknownRoleError
  ...
  if threshold is None or threshold <= 0:
      raise ssl_crypto.Error("Invalid threshold: " + str(threshold))
...
def may_need_new_keys(signature_status):
  ...
  <Arguments>
    signature_status:
      The dictionary returned by ssl_crypto.sig.get_signature_status().

  <Exceptions>
    ssl_crypto.FormatError, if 'signature_status does not have the correct format.
  ...
  # Raise 'ssl_crypto.FormatError' if the check fails.
  ssl_crypto.formats.SIGNATURESTATUS_SCHEMA.check_match(signature_status)
...
def generate_rsa_signature(signed, rsakey_dict):
  ...
  <Arguments>
    signed:
      The data used by 'ssl_crypto.keys.create_signature()' to generate signatures.
      It is stored in the 'signed' field of 'signable'.

    rsakey_dict:
      The RSA key, a 'ssl_crypto.formats.RSAKEY_SCHEMA' dictionary.
      Used here to produce 'keyid', 'method', and 'sig'.

  <Exceptions>
    ssl_crypto.FormatError, if 'rsakey_dict' does not have the correct format.
  ...
  <Returns>
    Signature dictionary conformant to ssl_crypto.formats.SIGNATURE_SCHEMA.
    Has the form:
  ...
  signed = ssl_crypto.formats.encode_canonical(signed)

  # Generate the RSA signature.
  # Raises ssl_crypto.FormatError and TypeError.
  signature = ssl_crypto.keys.create_signature(rsakey_dict, signed)
