...
# 'ssl_crypto.UnsupportedLibraryError' exception is raised.  
import ssl_crypto._vendor.ed25519.ed25519
...
def generate_public_and_private():
  ...
  <Purpose> 
    ...
    and private keys returned conform to 'ssl_crypto.formats.ED25519PULIC_SCHEMA' and
    'ssl_crypto.formats.ED25519SEED_SCHEMA', respectively, and have the form:
    ...
    >>> ssl_crypto.formats.ED25519PUBLIC_SCHEMA.matches(public)
    True
    >>> ssl_crypto.formats.ED25519SEED_SCHEMA.matches(private)
    True
  ...
  <Exceptions>
    ssl_crypto.UnsupportedLibraryError, if the PyNaCl ('nacl') module is unavailable.
  ...
  <Returns>
    A (public, private) tuple that conform to 'ssl_crypto.formats.ED25519PUBLIC_SCHEMA'
    and 'ssl_crypto.formats.ED25519SEED_SCHEMA', respectively.
  """
  ...
  try:
    ...
    raise ssl_crypto.UnsupportedLibraryError(message)
...
def create_signature(public_key, private_key, data):
  ...
  <Purpose>
    ...
    to 'ssl_crypto.formats.ED25519SIGNATURE_SCHEMA', and has the form:
  ...
  <Exceptions>
    ...
    ssl_crypto.CryptoError, if a signature cannot be created.
  ...
  # 'ssl_crypto.formats.ED25519PUBLIC_SCHEMA', which must have length 32 bytes.
  # Raise 'ssl_crypto.FormatError' if the check fails.
  ssl_crypto.formats.ED25519PUBLIC_SCHEMA.check_match(public_key)

  # Is 'private_key' properly formatted?
  ssl_crypto.formats.ED25519SEED_SCHEMA.check_match(private_key)
  ...
  # The private and public keys have been validated above by 'ssl_crypto.formats' and
  ...
  try:
    ...
    raise ssl_crypto.CryptoError(message + str(e))
...
def verify_signature(public_key, method, signature, data, use_pynacl=False):
  ...
  <Exceptions>
    ...
    ssl_crypto.FormatError. Raised if the arguments are improperly formatted. 
  ...
  ssl_crypto.formats.NAME_SCHEMA.check_match(method)
  
  # Is 'signature' properly formatted?
  ssl_crypto.formats.ED25519SIGNATURE_SCHEMA.check_match(signature)
  
  # Is 'use_pynacl' properly formatted?
  ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(use_pynacl)

  # Verify 'signature'.  Before returning the Boolean result,
  # ensure 'ed25519' was used as the signing method.
  # Raise 'ssl_crypto.UnsupportedLibraryError' if 'use_pynacl' is True but 'nacl' is
  # unavailable.
  public = public_key
  ...
  if method in _SUPPORTED_ED25519_SIGNING_METHODS:
    ...
    # Verify 'ed25519' signature with the pure Python implementation. 
      ...
      try:
        ssl_crypto._vendor.ed25519.ed25519.checkvalid(signature, data, public)
        valid_signature = True
    ...
    raise ssl_crypto.UnknownMethodError(message)
