...
logger = logging.getLogger('ssl_crypto.test_hash')


if not 'hashlib' in ssl_crypto.hash._supported_libraries:
  logger.warn('Not testing hashlib: could not be imported.')
if not 'pycrypto' in ssl_crypto.hash._supported_libraries:
  logger.warn('Not testing pycrypto: could not be imported.')

class TestHash(unittest.TestCase):

  def _run_with_all_hash_libraries(self, test_func):
    if 'hashlib' in ssl_crypto.hash._supported_libraries:
      test_func('hashlib')
    if 'pycrypto' in ssl_crypto.hash._supported_libraries:
      test_func('pycrypto')
  ...
  def _do_md5_update(self, library):
    digest_object = ssl_crypto.hash.digest('md5', library)
  ...
  def _do_sha1_update(self, library):
    digest_object = ssl_crypto.hash.digest('sha1', library)
  ...
  def _do_sha224_update(self, library):
    digest_object = ssl_crypto.hash.digest('sha224', library)
  ...
  def _do_sha256_update(self, library):
    digest_object = ssl_crypto.hash.digest('sha256', library)
    self.assertEqual(digest_object.hexdigest(),
  ...
  def _do_sha384_update(self, library):
    digest_object = ssl_crypto.hash.digest('sha384', library)
    self.assertEqual(digest_object.hexdigest(),
  ...
  def _do_sha512_update(self, library):
    digest_object = ssl_crypto.hash.digest('sha512', library)
  ...
  def _do_unsupported_algorithm(self, library):
    self.assertRaises(ssl_commons.exceptions.UnsupportedAlgorithmError, ssl_crypto.hash.digest, 'bogus')
  ...
  def _do_digest_size(self, library):
    self.assertEqual(16, ssl_crypto.hash.digest('md5', library).digest_size)
    self.assertEqual(20, ssl_crypto.hash.digest('sha1', library).digest_size)
    self.assertEqual(28, ssl_crypto.hash.digest('sha224', library).digest_size)
    self.assertEqual(32, ssl_crypto.hash.digest('sha256', library).digest_size)
    self.assertEqual(48, ssl_crypto.hash.digest('sha384', library).digest_size)
    self.assertEqual(64, ssl_crypto.hash.digest('sha512', library).digest_size)
  ...
  def _do_update_filename(self, library):
    ...
    try:
      ...
      for algorithm in ['md5', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512']:
        digest_object_truth = ssl_crypto.hash.digest(algorithm, library)
        digest_object_truth.update(data.encode('utf-8'))
        digest_object = ssl_crypto.hash.digest_filename(filename, algorithm, library)
  ...
  def _do_update_file_obj(self, library):
    ...
    for algorithm in ['md5', 'sha1', 'sha224', 'sha256', 'sha384', 'sha512']:
      digest_object_truth = ssl_crypto.hash.digest(algorithm, library)
      digest_object_truth.update(data.encode('utf-8'))
      digest_object = ssl_crypto.hash.digest_fileobject(file_obj, algorithm, library)
  ...
  def test_unsupported_digest_algorithm_and_library(self):
    self.assertRaises(ssl_commons.exceptions.UnsupportedAlgorithmError, ssl_crypto.hash.digest,
                      'sha123', 'hashlib')
    self.assertRaises(ssl_commons.exceptions.UnsupportedAlgorithmError, ssl_crypto.hash.digest,
                      'sha123', 'pycrypto')
    
    self.assertRaises(ssl_commons.exceptions.UnsupportedLibraryError, ssl_crypto.hash.digest,
                      'sha256', 'badlib')
