...
# 'remote_directory' entries.  See 'ssl_crypto/pushtools/pushtoolslib.py' and
# 'ssl_crypto/pushtools/push.py'.
...
# see 'ssl_crypto/pushtools/pushtoolslib.py' and 'ssl_crypto/pushtools/receive/receive.py'
...
class TargetsFile(MetaFile):
  ...
  def make_metadata(version, expiration_date, filedict=None, delegations=None):
    if filedict is None and delegations is None:
      raise ssl_crypto.Error('We don\'t allow completely empty targets metadata.')
...
def unix_timestamp_to_datetime(unix_timestamp):
  ...
  <Arguments>
    unix_timestamp:
      ...
      'ssl_crypto.formats.UNIX_TIMESTAMP_SCHEMA'.

  <Exceptions>
    ssl_crypto.FormatError, if 'unix_timestamp' is improperly formatted.
...
def make_fileinfo(length, hashes, custom=None):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the 'FILEINFO_SCHEMA' to be returned
  ...
  <Side Effects>
    ...
    will raise a 'ssl_crypto.FormatError' exception.
...
def make_versioninfo(version_number):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if the dict to be returned does not have the correct
  ...
  # Raise 'ssl_crypto.FormatError' if 'versioninfo' is improperly formatted.
  try: 
...
def get_role_class(expected_rolename):
  ...
  try:
    ...
    raise ssl_crypto.FormatError(repr(expected_rolename) + ' not supported.')
...
def expected_meta_rolename(meta_rolename):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'meta_rolename' is improperly formatted.
