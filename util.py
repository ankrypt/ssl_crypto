...
class TempFile(object):
  ...
  def _default_temporary_directory(self, prefix):
    ...
    try:
      ...
      raise ssl_crypto.Error(err)



  def __init__(self, prefix='ssl_crypto_temp_'):
    """
    <Purpose>
    ...
    <Exceptions>
      ssl_crypto.Error on failure to load temp dir.
    ...
    temp_dir = ssl_crypto.conf.temporary_directory
    if temp_dir is not None and ssl_crypto.formats.PATH_SCHEMA.matches(temp_dir):
      try:
        self.temporary_file = tempfile.NamedTemporaryFile(prefix=prefix,
                                                          dir=temp_dir)
  ...
  def read(self, size=None):
    ...
    <Exceptions>
      ssl_crypto.FormatError: if 'size' is invalid.
  ...
  def decompress_temp_file_object(self, compression):
    ...
    <Exceptions>
      ssl_crypto.FormatError: If 'compression' is improperly formatted.

      ssl_crypto.Error: If an invalid compression is given.

      ssl_crypto.DecompressionError: If the compression failed for any reason.
    ...
    ssl_crypto.formats.NAME_SCHEMA.check_match(compression)
    
    if self._orig_file is not None:
      raise ssl_crypto.Error('Can only set compression on a TempFile once.')

    if compression != 'gzip':
      raise ssl_crypto.Error('Only gzip compression is supported.')
    ...
    try:
      ...
      raise ssl_crypto.DecompressionError(exception)
...
def get_file_details(filepath, hash_algorithms=['sha256']):
  ...
  <Exceptions>
    ssl_crypto.FormatError: If hash of the file does not match HASHDICT_SCHEMA.

    ssl_crypto.Error: If 'filepath' does not exist. 
  ...
  # 'ssl_crypto.FormatError' is raised on incorrect format.
  ssl_crypto.formats.PATH_SCHEMA.check_match(filepath)
  ssl_crypto.formats.HASHALGORITHMS_SCHEMA.check_match(hash_algorithms)
  ...
  if not os.path.exists(filepath):
    raise ssl_crypto.Error('Path ' + repr(filepath) + ' doest not exist.')
  ...
  for algorithm in hash_algorithms:
    digest_object = ssl_crypto.hash.digest_filename(filepath, algorithm)
  ...
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.HASHDICT_SCHEMA.check_match(file_hashes)
...
def ensure_parent_dir(filename):
  ...
  # Raise 'ssl_crypto.FormatError' on a mismatch.
...
def file_in_confined_directories(filepath, confined_directories):
  ...
  <Exceptions>
   ssl_crypto.FormatError: On incorrect format of the input.
  ...
  ssl_crypto.formats.RELPATHS_SCHEMA.check_match(confined_directories)
...
def find_delegated_role(roles, delegated_role):
  ...
  <Exceptions>
    ssl_crypto.RepositoryError, if the list of roles has invalid data.
  ...
  ssl_crypto.formats.ROLELIST_SCHEMA.check_match(roles)
  ssl_crypto.formats.ROLENAME_SCHEMA.check_match(delegated_role)
  ...
  for index in six.moves.xrange(len(roles)):
    ...
    if name is None:
      ...
      raise ssl_crypto.RepositoryError(no_name_message)
    ...
    # Does this role have the same name?
      ...
      if name == delegated_role:
        ...
        # ...there are at least two roles with the same name.
          ...
          raise ssl_crypto.RepositoryError(duplicate_role_message)
...
def paths_are_consistent_with_hash_prefixes(paths, path_hash_prefixes):
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
  ssl_crypto.formats.RELPATHS_SCHEMA.check_match(paths)
...
def get_target_hash(target_filepath):
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.RELPATH_SCHEMA.check_match(target_filepath)

  # Calculate the hash of the filepath to determine which bin to find the 
  # target.  The client currently assumes the repository uses
  # 'HASH_FUNCTION' to generate hashes and 'utf-8'.
  digest_object = ssl_crypto.hash.digest(HASH_FUNCTION)
...
def load_json_string(data):
  ...
  <Exceptions>
    ssl_crypto.Error, if 'data' cannot be deserialized to a Python object.
...
def load_json_file(filepath):
  ...
  <Exceptions>
    ...
    ssl_crypto.Error: If 'filepath' cannot be deserialized to a Python object.
...
def digests_are_equal(digest1, digest2):
  ...
  # types, and that all dict keys are properly named.
  # Raise 'ssl_crypto.FormatError' if there is a mismatch.
  ssl_crypto.formats.HEX_SCHEMA.check_match(digest1)
  ssl_crypto.formats.HEX_SCHEMA.check_match(digest2)
