...
<Purpose>
  ...
  module.  See 'ssl_crypto/README' for the complete guide to using
  'ssl_crypto.repository_tool.py'.
"""
...
import ssl_crypto.repository_lib as repo_lib
from ssl_crypto.repository_lib import generate_and_write_rsa_keypair
from ssl_crypto.repository_lib import generate_and_write_ed25519_keypair
from ssl_crypto.repository_lib import import_rsa_publickey_from_file
from ssl_crypto.repository_lib import import_ed25519_publickey_from_file
from ssl_crypto.repository_lib import import_rsa_privatekey_from_file
from ssl_crypto.repository_lib import import_ed25519_privatekey_from_file
from ssl_crypto.repository_lib import create_ssl_crypto_client_directory
from ssl_crypto.repository_lib import disable_console_log_messages 
...
logger = logging.getLogger('ssl_crypto.repository_tool')

# Add a console handler so that users are aware of potentially unintended
# states, such as multiple roles that share keys.
ssl_crypto.log.add_console_handler()
ssl_crypto.log.set_console_log_level(logging.INFO)
...
try:
  ssl_crypto.keys.check_crypto_libraries(['rsa', 'ed25519', 'general'])

except ssl_crypto.UnsupportedLibraryError: #pragma: no cover
  logger.warn('Warning: The repository and developer tools require'
    ' additional libraries, which can be installed as follows:'
    '\n $ pip install ssl_crypto[tools]') 
...
class Repository(object):
  ...
  def write(self, write_partial=False, consistent_snapshot=False,
    ...
    <Exceptions>
      ssl_crypto.UnsignedMetadataError, if any of the top-level and delegated roles do
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
    ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(write_partial)
    ...
    # At this point the ssl_crypto.keydb and ssl_crypto.roledb stores must be fully
    # populated, otherwise write() throwns a 'ssl_crypto.UnsignedMetadataError'
    ...
    delegated_rolenames = ssl_crypto.roledb.get_delegated_rolenames('targets')
    for delegated_rolename in delegated_rolenames:
      ...
      roleinfo = ssl_crypto.roledb.get_roleinfo(delegated_rolename)
      delegated_targets = list(roleinfo['paths'].keys())
      parent_rolename = ssl_crypto.roledb.get_parent_rolename(delegated_rolename)
      parent_roleinfo = ssl_crypto.roledb.get_roleinfo(parent_rolename) 
      parent_delegations = parent_roleinfo['delegations']
      
      # Raise exception if any of the targets of 'delegated_rolename' are not
      # allowed.
      ssl_crypto.util.ensure_all_targets_allowed(delegated_rolename, delegated_targets,
                                          parent_delegations)
      ...
      # sub-directory.
      ssl_crypto.util.ensure_parent_dir(delegated_filename)
    ...
    # _generate_and_write_metadata() raises a 'ssl_crypto.Error' exception if the
    ...
    # Delete the metadata of roles no longer in 'ssl_crypto.roledb'.  Obsolete roles
  ...
  def status(self):
    ...
    try:
      ...
      # targets role.
      delegated_roles = ssl_crypto.roledb.get_delegated_rolenames('targets')
      insufficient_keys = []
      ...
      for delegated_role in delegated_roles:
        ...
        ssl_crypto.util.ensure_parent_dir(filename)
        ...
        except ssl_crypto.InsufficientKeysError:
        ...
        try: 
          repo_lib._generate_and_write_metadata(delegated_role, filename, False,
                                                targets_directory,
                                                metadata_directory)
        except ssl_crypto.UnsignedMetadataError:
  ...
  def get_filepaths_in_directory(files_directory, recursive_walk=False,
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'file_directory' is not a valid directory.
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
    ssl_crypto.formats.PATH_SCHEMA.check_match(files_directory)
    ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(recursive_walk)
    ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(followlinks)

    # Ensure a valid directory is given.
    if not os.path.isdir(files_directory):
      raise ssl_crypto.Error(repr(files_directory) + ' is not a directory.')
...
class Metadata(object):
  ...
  def add_verification_key(self, key):
    ...
    <Arguments>
      key:
        The role key to be added, conformant to 'ssl_crypto.formats.ANYKEY_SCHEMA'.
    ...
    <Exceptions>
      ssl_crypto.FormatError, if the 'key' argument is improperly formatted.    

    <Side Effects>
      The role's entries in 'ssl_crypto.keydb.py' and 'ssl_crypto.roledb.py' are updated.
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
    ssl_crypto.formats.ANYKEY_SCHEMA.check_match(key)

    # Ensure 'key', which should contain the public portion, is added to
    # 'ssl_crypto.keydb.py'.  Add 'key' to the list of recognized keys.  Keys may be
    # shared, so do not raise an exception if 'key' has already been loaded.
    try:
      ssl_crypto.keydb.add_key(key)
    
    except ssl_crypto.KeyAlreadyExistsError:
      logger.warning('Adding a verification key that has already been used.')

    keyid = key['keyid']
    roleinfo = ssl_crypto.roledb.get_roleinfo(self.rolename)
   
    # Add 'key' to the role's entry in 'ssl_crypto.roledb.py' and avoid duplicates.
    if keyid not in roleinfo['keyids']: 
      roleinfo['keyids'].append(keyid)
      
      ssl_crypto.roledb.update_roleinfo(self._rolename, roleinfo)
  ...
  def remove_verification_key(self, key):
    ...
    <Arguments>
      key:
        The role's key, conformant to 'ssl_crypto.formats.ANYKEY_SCHEMA'.  'key'
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if the 'key' argument has not been previously added.
    
    <Side Effects>
      Updates the role's 'ssl_crypto.roledb.py' entry.
    ...
    if keyid in roleinfo['keyids']: 
      ...
      raise ssl_crypto.Error('Verification key not found.')
  ...
  def load_signing_key(self, key):
    ...
    <Arguments>
      key:
        The role's key, conformant to 'ssl_crypto.formats.ANYKEY_SCHEMA'.  It must
    ...
    <Exceptions>
      ssl_crypto.FormatError, if 'key' is improperly formatted.

      ssl_crypto.Error, if the private key is not found in 'key'.

    <Side Effects>
      Updates the role's 'ssl_crypto.keydb.py' and 'ssl_crypto.roledb.py' entries.
    ...
    if not len(key['keyval']['private']):
      raise ssl_crypto.Error('This is not a private key.')
    ...
    try:
      ...
      ssl_crypto.keydb.remove_key(key['keyid'])
      ssl_crypto.keydb.add_key(key)

    # Update the role's 'signing_keys' field in 'ssl_crypto.roledb.py'.
    roleinfo = ssl_crypto.roledb.get_roleinfo(self.rolename)
    if key['keyid'] not in roleinfo['signing_keyids']:
      roleinfo['signing_keyids'].append(key['keyid'])
      
      ssl_crypto.roledb.update_roleinfo(self.rolename, roleinfo)
  ...
  def unload_signing_key(self, key):
    ...
    <Arguments>
      key:
        The role key to be unloaded, conformant to 'ssl_crypto.formats.ANYKEY_SCHEMA'.

    <Exceptions>
      ssl_crypto.FormatError, if the 'key' argument is improperly formatted.

      ssl_crypto.Error, if the 'key' argument has not been previously loaded.

    <Side Effects>
      Updates the signing keys of the role in 'ssl_crypto.roledb.py'.
    ...
    if key['keyid'] in roleinfo['signing_keyids']:
      ...
      raise ssl_crypto.Error('Signing key not found.')
  ...
  def add_signature(self, signature):
    ...
    <Arguments>
      signature:
        ...
        'ssl_crypto.formats.SIGNATURE_SCHEMA'.

    <Exceptions>
      ssl_crypto.FormatError, if the 'signature' argument is improperly formatted.

    <Side Effects>
      Adds 'signature', if not already added, to the role's 'signatures' field
      in 'ssl_crypto.roledb.py'.
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
    ssl_crypto.formats.SIGNATURE_SCHEMA.check_match(signature)
  ...
  def remove_signature(self, signature):
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'signature' has not been previously added to this role.

    <Side Effects>
      Updates the 'signatures' field of the role in 'ssl_crypto.roledb.py'.
    ...
    if signature in roleinfo['signatures']:
      ...
      raise ssl_crypto.Error('Signature not found.')
  ...
  def signatures(self):
    ...
    <Returns>
      A list of signatures, conformant to 'ssl_crypto.formats.SIGNATURES_SCHEMA'.
    """
  ...
  def rolename(self):
    ...
    <Returns>
      The role's name, conformant to 'ssl_crypto.formats.ROLENAME_SCHEMA'.
      Examples: 'root', 'timestamp', 'targets/unclaimed/django'.
    """
  ...
  def version(self):
    ...
    <Purpose>
      ...
      'ssl_crypto.formats.VERSION_SCHEMA'.
    ...
    <Returns>
      The role's version number, conformant to 'ssl_crypto.formats.VERSION_SCHEMA'.
    """
  ...
  def version(self, version):
    ...
    <Exceptions>
      ssl_crypto.FormatError, if the 'version' argument is improperly formatted.

    <Side Effects>
      Modifies the 'version' attribute of the Repository object and updates
      the role's version in 'ssl_crypto.roledb.py'.
  ...
  def threshold(self):
    ...
    <Returns>
      The role's threshold value, conformant to 'ssl_crypto.formats.THRESHOLD_SCHEMA'.
    """
    
    roleinfo = ssl_crypto.roledb.get_roleinfo(self._rolename)
  ...
  def threshold(self, threshold):
    ...
    <Arguments>
      threshold:
        ...
        signed.  Conformant to 'ssl_crypto.formats.THRESHOLD_SCHEMA'.

    <Exceptions>
      ssl_crypto.FormatError, if the 'threshold' argument is improperly formatted.

    <Side Effects>
      Modifies the threshold attribute of the Repository object and updates
      the roles threshold in 'ssl_crypto.roledb.py'.
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
    ssl_crypto.formats.THRESHOLD_SCHEMA.check_match(threshold)
  ...
  def expiration(self, datetime_object):
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'datetime_object' has already expired.
    ...
    if not isinstance(datetime_object, datetime.datetime):
      raise ssl_crypto.FormatError(repr(datetime_object) + ' is not a'
    ...
    current_datetime_object = \
      ssl_crypto.formats.unix_timestamp_to_datetime(int(time.time()))
    
    if datetime_object < current_datetime_object:
      raise ssl_crypto.Error(repr(self.rolename) + ' has already expired.')
   
    # Update the role's 'expires' entry in 'ssl_crypto.roledb.py'.
    roleinfo = ssl_crypto.roledb.get_roleinfo(self.rolename)
  ...
  def signing_keys(self):
    ...
    <Returns>
      ...
      'ssl_crypto.formats.KEYIDS_SCHEMA'.
    """
  ...
  def compressions(self):
    ...
    <Returns>
      ...
      'ssl_crypto.formats.COMPRESSIONS_SCHEMA'.
    """
  ...
  def compressions(self, compression_list):
    ...
    <Exceptions>
      ssl_crypto.FormatError, if 'compression_list' is improperly formatted.

    <Side Effects>
      Updates the role's compression algorithms listed in 'ssl_crypto.roledb.py'.
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if any are improperly formatted.
    ssl_crypto.formats.COMPRESSIONS_SCHEMA.check_match(compression_list)

    roleinfo = ssl_crypto.roledb.get_roleinfo(self.rolename)
   
    # Add the compression algorithms of 'compression_list' to the role's
    # entry in 'ssl_crypto.roledb.py'.
...
class Root(Metadata):
  ...
  <Side Effects>
    A 'root' role is added to 'ssl_crypto.roledb.py'.
  ...
  def __init__(self):
    ...
    expiration = \
      ssl_crypto.formats.unix_timestamp_to_datetime(int(time.time() + ROOT_EXPIRATION))
    ...
    try: 
      ssl_crypto.roledb.add_role(self._rolename, roleinfo)
    
    except ssl_crypto.RoleAlreadyExistsError:
      pass
...
class Timestamp(Metadata):
  ...
  <Side Effects>
    A 'timestamp' role is added to 'ssl_crypto.roledb.py'.
  ...
  def __init__(self):
    ...
    # time.  The expiration may be modified.
    expiration = \
      ssl_crypto.formats.unix_timestamp_to_datetime(int(time.time() + TIMESTAMP_EXPIRATION))
    expiration = expiration.isoformat() + 'Z'
    ...
    try: 
      ssl_crypto.roledb.add_role(self.rolename, roleinfo)
...
class Snapshot(Metadata):
  ...
  <Side Effects>
    A 'snapshot' role is added to 'ssl_crypto.roledb.py'.
  ...
  def __init__(self):
    ...
    # time.  The expiration may be modified.
    expiration = \
      ssl_crypto.formats.unix_timestamp_to_datetime(int(time.time() + SNAPSHOT_EXPIRATION))
    expiration = expiration.isoformat() + 'Z'
...
class Targets(Metadata):
  ...
  <Arguments>
    ...
    roleinfo:
      ...
      'ssl_crypto.formats.ROLEDB_SCHEMA'.

  <Exceptions>
    ssl_crypto.FormatError, if the arguments are improperly formatted.

  <Side Effects>
    Modifies the roleinfo of the targets role in 'ssl_crypto.roledb', or creates
  ...
  def __init__(self, targets_directory, rolename='targets', roleinfo=None):
    ...
    if roleinfo is not None:
      ssl_crypto.formats.ROLEDB_SCHEMA.check_match(roleinfo)
    ...
    expiration = \
      ssl_crypto.formats.unix_timestamp_to_datetime(int(time.time() + TARGETS_EXPIRATION))
    expiration = expiration.isoformat() + 'Z'
    ...
    # Add the new role to the 'ssl_crypto.roledb'.
    try:
      ssl_crypto.roledb.add_role(self.rolename, roleinfo)
  ...
  def __call__(self, rolename):
    ...
    <Exceptions>
      ...
      ssl_crypto.UnknownRoleError, if 'rolename' has not been delegated by this
      Targets object.

    <Side Effects>
      Modifies the roleinfo of the targets role in 'ssl_crypto.roledb'.
    ...
    if rolename in self._delegated_roles:
      ...
      raise ssl_crypto.UnknownRoleError(repr(rolename) + ' has not been delegated'
  ...
  def target_files(self):
    ...
    target_files = ssl_crypto.roledb.get_roleinfo(self._rolename)['paths']
  ...
  def add_restricted_paths(self, list_of_directory_paths, child_rolename):
    ...
    <Exceptions>
      ssl_crypto.Error, if a directory path in 'list_of_directory_paths' is not a
    ...
    ssl_crypto.formats.PATHS_SCHEMA.check_match(list_of_directory_paths)
    ssl_crypto.formats.ROLENAME_SCHEMA.check_match(child_rolename)
    ...
    if not ssl_crypto.roledb.role_exists(full_child_rolename):
      raise ssl_crypto.Error(repr(full_child_rolename) + ' has not been delegated.')

    # Are the paths in 'list_of_directory_paths' valid?
    for directory_path in list_of_directory_paths:
      directory_path = os.path.abspath(directory_path)
      if not os.path.isdir(directory_path):
        raise ssl_crypto.Error(repr(directory_path) + ' is not a directory.')
      ...
      if not directory_path.startswith(targets_directory):
        raise ssl_crypto.Error(repr(directory_path) + ' is not under the'
  ...
  def add_target(self, filepath, custom=None):
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'filepath' is not found under the repository's targets
      directory.

    <Side Effects>
      Adds 'filepath' to this role's list of targets.  This role's
      'ssl_crypto.roledb.py' is also updated.
    ...
    if not filepath.startswith(self._targets_directory): 
      raise ssl_crypto.Error(repr(filepath) + ' is not under the Repository\'s'
    ...
    if os.path.isfile(filepath):
      ...
      # Update the role's 'ssl_crypto.roledb.py' entry and avoid duplicates.
      ...
      raise ssl_crypto.Error(repr(filepath) + ' is not a valid file.')
  ...
  def add_targets(self, list_of_targets):
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if any of the paths listed in 'list_of_targets' is not found
    ...
    # Update the ssl_crypto.roledb entry.
    targets_directory_length = len(self._targets_directory) 
    ...
    # Update this Targets 'ssl_crypto.roledb.py' entry.
    roleinfo = ssl_crypto.roledb.get_roleinfo(self._rolename)
  ...
  def remove_target(self, filepath):
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'filepath' is not under the repository's targets directory,
      or not found.

    <Side Effects>
      Modifies this Targets 'ssl_crypto.roledb.py' entry.
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if there is a mismatch.
    ssl_crypto.formats.RELPATH_SCHEMA.check_match(filepath)
    ...
    fileinfo = ssl_crypto.roledb.get_roleinfo(self.rolename)
    if relative_filepath in fileinfo['paths']:
      del fileinfo['paths'][relative_filepath]
      ssl_crypto.roledb.update_roleinfo(self.rolename, fileinfo)
    
    else:
      raise ssl_crypto.Error('Target file path not found.')
  ...
  def get_delegated_rolenames(self):
    ...
    return ssl_crypto.roledb.get_delegated_rolenames(self.rolename)
  ...
  def delegate(self, rolename, public_keys, list_of_targets, threshold=1,
    ...
    <Arguments>
      ...
      backtrack:
        ...
        this role, a 'ssl_crypto.UnknownTargetError' exception should be raised.  If
      ...
      path_hash_prefixes:
        A list of hash prefixes in 'ssl_crypto.formats.PATH_HASH_PREFIXES_SCHEMA'
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if the delegated role already exists or if any of the arguments
    ...
    <Side Effects>
      ...
      caller (i.e., targets.unclaimed.<rolename>).  The 'ssl_crypto.keydb.py' and
      'ssl_crypto.roledb.py' stores are updated with 'public_keys'.
    ...
    # Raise 'ssl_crypto.FormatError' if there is a mismatch.
    ssl_crypto.formats.ROLENAME_SCHEMA.check_match(rolename)
    ssl_crypto.formats.ANYKEYLIST_SCHEMA.check_match(public_keys)
    ssl_crypto.formats.RELPATHS_SCHEMA.check_match(list_of_targets)
    ssl_crypto.formats.THRESHOLD_SCHEMA.check_match(threshold)
    ssl_crypto.formats.BOOLEAN_SCHEMA.check_match(backtrack)
    
    if restricted_paths is not None:
      ssl_crypto.formats.RELPATHS_SCHEMA.check_match(restricted_paths)
    
    if path_hash_prefixes is not None:
      ssl_crypto.formats.PATH_HASH_PREFIXES_SCHEMA.check_match(path_hash_prefixes)
   

    # Check if 'rolename' is not already a delegation.  'ssl_crypto.roledb' expects the
    # full rolename. 
    full_rolename = self._rolename + '/' + rolename

    if ssl_crypto.roledb.role_exists(full_rolename):
      raise ssl_crypto.Error(repr(rolename) + ' already delegated.')
    ...
    # Add all the keys of 'public_keys' to ssl_crypto.keydb.
    for key in public_keys:
      keyid = key['keyid']
      key_metadata_format = ssl_crypto.keys.format_keyval_to_metadata(key['keytype'],
                                                               key['keyval'])
    ...
    for target in list_of_targets:
      ...
      if not target.startswith(self._targets_directory+os.sep):
        raise ssl_crypto.Error(repr(target) + ' is not under the Repository\'s'
          ' targets directory: ' + repr(self._targets_directory))
    ...
    if restricted_paths is not None: 
      for path in restricted_paths:
        ...
        if not path.startswith(self._targets_directory + os.sep):
          raise ssl_crypto.Error(repr(path) + ' is not under the Repository\'s'
            ' targets directory: ' +repr(self._targets_directory))
    ...
    current_roleinfo = ssl_crypto.roledb.get_roleinfo(self.rolename) 
    ...
    ssl_crypto.roledb.update_roleinfo(self.rolename, current_roleinfo)  
  ...
  def revoke(self, rolename):
    ...
    <Side Effects>
      The delegations dictionary of 'rolename' is modified, and its 'ssl_crypto.roledb'
    ...
    # Remove 'rolename' from 'ssl_crypto.roledb.py'.  The delegations of 'rolename' are
    # also removed.
    ssl_crypto.roledb.remove_role(full_rolename)
  ...
  def delegate_hashed_bins(self, list_of_targets, keys_of_hashed_bins,
    ...
    <Purpose>
      ...
      files (and their size) downloaded by the client.  See ssl_crypto-spec.txt and the
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'number_of_bins' is not a power of 2, or one of the targets
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if there is a mismatch.
    ssl_crypto.formats.PATHS_SCHEMA.check_match(list_of_targets)
    ssl_crypto.formats.ANYKEYLIST_SCHEMA.check_match(keys_of_hashed_bins)
    ssl_crypto.formats.NUMBINS_SCHEMA.check_match(number_of_bins)
    ...
    if total_hash_prefixes % number_of_bins != 0:
      raise ssl_crypto.Error('The "number_of_bins" argument must be a power of 2.')
    ...
    for target_path in list_of_targets:
      ...
      if not target_path.startswith(self._targets_directory+os.sep):
        raise ssl_crypto.Error('A path in the list of targets argument is not'
      ...
      digest_object = ssl_crypto.hash.digest(algorithm=HASH_FUNCTION)
  ...
  def add_target_to_bin(self, target_filepath):
    ...
    <Exceptions>
      ssl_crypto.FormatError, if 'target_filepath' is improperly formatted.
      
      ssl_crypto.Error, if 'target_filepath' cannot be added to a hashed bin
    ...
    # types, and that all dict keys are properly named.
    # Raise 'ssl_crypto.FormatError' if there is a mismatch.
    ssl_crypto.formats.PATH_SCHEMA.check_match(target_filepath)
  ...
  def remove_target_from_bin(self, target_filepath):
    ...
    <Exceptions>
      ...
      ssl_crypto.Error, if 'target_filepath' cannot be removed from a hashed bin
      (e.g., an invalid target filepath, or the expected hashed bin does not
      exist.)
  ...
  def _locate_and_update_target_in_bin(self, target_filepath, method_name):
    ...
    <Exceptions>
      ssl_crypto.Error, if 'target_filepath' cannot be updated (e.g., an invalid target
    ...
    if len(roleinfo['delegations']['roles']):
      ...
      raise ssl_crypto.Error(self.rolename + ' has not delegated to any roles.')
    ...
    if 'path_hash_prefixes' in delegation and len(delegation['path_hash_prefixes']):
      ...
      raise ssl_crypto.Error(self.rolename + ' has not delegated to hashed bins.')
    ...
    if hashed_bin_name is not None:
      ...
      raise ssl_crypto.Error(target_filepath + ' not found in any of the bins.')
  ...
  def delegations(self):
    ...
    <Exceptions>
      ssl_crypto.UnknownRoleError, if this Targets' rolename does not exist in
      'ssl_crypto.roledb'. 
...
def create_new_repository(repository_directory):
  ...
  <Returns>
    A 'ssl_crypto.repository_tool.Repository' object.
  """
...
def load_repository(repository_directory):
  ...
  <Exceptions>
    ssl_crypto.FormatError, if 'repository_directory' or any of the metadata files
    are improperly formatted.

    ssl_crypto.RepositoryError, if the Root role cannot be found.  At a minimum,
  ...
  if os.path.exists(targets_metadata_directory) and \
    ...
    for root, directories, files in os.walk(targets_metadata_directory):
      ...
      for basename in files:
        ...
        signable = None
        try:
          signable = ssl_crypto.util.load_json_file(metadata_path)
        ...
        roleinfo = ssl_crypto.roledb.get_roleinfo(metadata_name)
        ...
        ssl_crypto.roledb.update_roleinfo(metadata_name, roleinfo)
        ...
        targets_object = \
          targets_objects[ssl_crypto.roledb.get_parent_rolename(metadata_name)]
