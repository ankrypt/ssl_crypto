...
<Purpose>
  ...
  'ssl_crypto' logging, they should do the following:

  import logging
  logger = logging.getLogger('ssl_crypto')
  ...
  'ssl_crypto'.  The 'log.py' module should only be imported once by the main program.
  When any other module does a logging.getLogger('ssl_crypto'), it is referring to the
  same 'ssl_crypto' instance, and its associated settings, set here in 'log.py'.
...
# [2013-08-13 15:21:18,068 localtime] [ssl_crypto] [INFO][_update_metadata:851@updater.py]
...
# be done with ssl_crypto.log.add_console_handler(). Logging messages to a file *is*
# set by default.
...
# 'ssl_crypto.conf.LOG_FILENAME', and only those messages with a log level of
# '_DEFAULT_LOG_LEVEL'.  The log level of messages handled by 'file_handler'
# may be modified with 'set_filehandler_log_level()'.  'ssl_crypto.conf.LOG_FILENAME'
# will be opened in append mode.
if ssl_crypto.conf.ENABLE_FILE_LOGGING:
  file_handler = logging.FileHandler(ssl_crypto.conf.LOG_FILENAME)
...
def set_log_level(log_level=_DEFAULT_LOG_LEVEL):
  ...
  ssl_crypto.formats.LOGLEVEL_SCHEMA.check_match(log_level)
...
def set_console_log_level(log_level=_DEFAULT_CONSOLE_LOG_LEVEL):
  ...
  <Exceptions>
    ssl_crypto.Error, if the 'log.py' console handler has not been set yet with
    add_console_handler().
